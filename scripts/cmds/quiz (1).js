...
config: {
  ...
  category: "game",
  guide: {
    en: ".quiz list | top | category | withdraw"
  }
},
  ...
  onStart: async function ({ event, message, usersData, api, args }) {
    if (args.length === 2 && args[0] === "withdraw") {
      ...
      if (isNaN(amount) || amount <= 0) {
        return message.reply("⛔ 𝗜𝗻𝘃𝗮𝗹𝗶𝗱 𝗔𝗺𝗼𝘂𝗻𝘁\n\nPlease provide a valid amount to withdraw.");
      }

      const userID = event.senderID;
      const userData = await getUserData(userID);
      if (!userData || userData.money < amount) {
        return message.reply("⛔ 𝗡𝗼 𝗗𝗮𝘁𝗮\n\nYou don't have enough money to withdraw.");
      }

      userData.money -= amount;
      await saveUserData(userID, userData);

      return message.reply(`✅ 𝗗𝗼𝗻𝗲\n\nSuccessfully withdrew $${amount} from your quiz wallet.`);
    }
...
async function fetchQuiz(category) {
  try {
    const response = await axios.get(`https://globalapis.onrender.com/api/quiz?category=${category}`);
